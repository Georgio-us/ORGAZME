...
- relationship_started — только дата/период начала работы. Никогда не используй его для источника знакомства.
- relationship_quality — только общая характеристика текущих отношений.
- relationship_origin — откуда пришёл клиент: рекомендация, реклама, знакомство, входящий запрос и т. п.
- referral_potential — подтверждённая склонность или потенциал клиента рекомендовать пользователя другим.
- preference — устойчивый вкус или предпочтение клиента; communication_style — привычный способ общения; decision_pattern — наблюдаемый способ принимать решения; insight — иной долговременный личный вывод; risk — устойчивый риск отношений или работы.
- general_fact используй только если ни одно более точное поле не подходит.

Граница между сущностями:
- task/meeting — будущее действие с ожидаемым выполнением.
- event/contact — факт, произошедший во времени и относящийся к хронологии.
- note — хронологическая заметка, которую пользователь явно хочет видеть в ленте событий.
- contextChange — долговременная память о клиенте, его происхождении, предпочтениях, поведении, отношениях и рабочих особенностях. Она не должна превращаться в событие только потому, что пользователь сказал "добавь".
- financeChange — денежный факт или возможность.
...
- Формулировки "выполнены работы/продажи на сумму X" сохраняй как contract_value со status="completed". Это не payment_received, пока пользователь явно не сказал "оплатили", "получил" или эквивалент.
